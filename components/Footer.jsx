...
const Footer = () => {
    ...
    const linkSections = [
        {
            ...
            links: [
                ...
                { text: "Smart Watches", path: '/shop?search=watch' },
    ...
    return (
        <footer className="mx-6 bg-white dark:bg-slate-900">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
                    <div>
                        ...
                        <p className="max-w-[380px] mt-6 text-sm leading-relaxed">Welcome to GoCart — your ultimate destination for the latest gadgets. From smartphones to smart accessories, we bring you the best in innovation all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-10 h-10 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-green-500 hover:text-white dark:hover:bg-green-500 dark:hover:text-white hover:scale-105 transition-all rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[50%] gap-8 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-semibold text-slate-700 dark:text-slate-300 md:mb-5 mb-3 tracking-wide text-xs">{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <span className="text-slate-400 dark:text-slate-500"><link.icon /></span>}
                                            <Link href={link.path} className="hover:text-green-500 dark:hover:text-green-400 transition-colors">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-5 text-sm text-slate-400 dark:text-slate-500 text-center">
                    © 2025 GoCart. All rights reserved. Built with ❤️ for smart shoppers.
                </p>
            </div>
        </footer>
    );
